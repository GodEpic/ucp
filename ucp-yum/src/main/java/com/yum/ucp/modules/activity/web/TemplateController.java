...
public class TemplateController extends DscBaseController {
    ...
    public String list(
        ...
        if (user.getBrand() != null && !"".equals(user.getBrand())) {
            template.setBrand(user.getBrand());
        }
    ...
    @RequestMapping("uploadTemplateFtpAttach")
    @ResponseBody
    public ResponseMessage uploadTemplateFtpAttach(@RequestParam("file") MultipartFile file) {
        try {
            JSONObject object = FtpUtils.uploadTemplateSignalFile(file, "template");
