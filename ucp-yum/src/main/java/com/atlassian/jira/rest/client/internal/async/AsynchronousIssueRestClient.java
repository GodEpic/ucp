...
public class AsynchronousIssueRestClient extends AbstractAsynchronousRestClient implements IssueRestClient {
	...
	public Promise<Iterable<Transition>> getTransitions(final URI transitionsUri) {
		return callAndParse(client().newRequest(transitionsUri).get(),
				new ResponseHandler<Iterable<Transition>>() {
					@Override
	...
	@Override
	public Promise<Void> deleteAttachment(URI attachmentsUri) {
		return delete(attachmentsUri);
	}
	...
	@Override
	public Promise<Void> deleteComment(URI commentsUri) {
		return delete(commentsUri);
	}
	@Override
	public Promise<Void> updateComment(final URI commentsUri,  Comment comment) {
		return put(commentsUri, comment, new CommentJsonGenerator(getVersionInfo()));
	}
