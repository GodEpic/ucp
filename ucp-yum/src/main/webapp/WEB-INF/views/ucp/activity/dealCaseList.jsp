                            ...
                            <c:if test="${issue.type ne '1'}">
                                <c:choose>
                                    ...
                                    <c:when test="${issue.status eq actStatusNotPass || issue.status eq actStatusNotTested || issue.status eq actStatusTesting || issue.status eq actStatusPass || issue.status eq actStatusTestFeedBack}">
                                        ...
                                        <c:if test="${roleType == 6}">
                                            <a href="javascript:void(0)" onclick="delActivity('${issue.id}')">删除</a>
                                        </c:if>
                                    </c:when>
                                    <c:otherwise>
                                        <a target="_blank" href="${ctx}/activity/form?id=${issue.id}">编辑</a>&nbsp;
                                        <a href="javascript:void(0)" onclick="delActivity('${issue.id}')">删除</a>
                                    </c:otherwise>
                                </c:choose>
                            </c:if>
...
<script type="text/javascript">
    ...
    function page(n, s) {
        if (n) $("#pageNo").val(n);
        if (s) $("#pageSize").val(s);
        $("#searchForm").attr("action", "${ctx}/activity/dealList?sort=" + $(".sort").val() + "&sortBy=" + $(".sortBy").val());
        $("#searchForm").submit();
        return false;
    }
    function sortPage(n) {
