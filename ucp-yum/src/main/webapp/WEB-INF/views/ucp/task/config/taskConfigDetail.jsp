	...
	<body>
		<div class="tab-con sec-case-form">
			...
			<div class="sec-case-forms">
							...
							<tbody>
								<c:forEach items="${modRowLists}" var="rw" varStatus="status">
									<tr>
										...
										<td><a href="javascript:void(0)" class="row-show" data-value="${rw.configCertifiCenter}" data-rowid="${rw.id}">查看</a></td>
										<td><div id="${rw.id}">${fns:getModuleActRowStatus(rw.configStatus)}</div></td>
				...
				<div class="sec" style="display:none;">
					<article class="sec-title t2">反馈记录</article>
					<div class="sec-form">
					            ...
					            <table class="table table-striped table-hover table-condensed" style="table-layout: fixed">
					                ...
					                <tbody id="comment_find" >
					                	<c:forEach items="${comments}" var="comment">
						                    <tr data-uri="${comment.self}">
						                        <td><div class="comment-body">${comment.body}</div></td>
						                        <td><div class="comment-user">${comment.author.name}</div></td>
						                        <td><div class="comment-date">${comment.creationDate}</div></td>
						                        <td><a href="javascript:void(0)" class="comment-remove btn btn-danger btn-small">删除</a></td>
						                    </tr>
						                </c:forEach>
					                </tbody>
					            </table>
					        </div>
					        <span id="add_comment"> <a id="addCommentBtn" class="btn btn-primary">添加反馈</a></span>
					    </div>
					</div>
				</div>
				...
				<div class="sec">
					<div class="sec-btn">
						<c:if test="${taskReceive.receive eq agent && fns:getDictLabel(task.configStatus, 'config_status', '') ne '已完成'}">
							<div class="sec-btn-submit"><a id="submitBtn" href="javascript:void(0)" class="layui-yum-btn">提交至下一步</a></div>
						</c:if>
		...
		<script type="text/javascript">
			$(function(){
				$("#addCommentBtn").click(function(){
					...
					var source = html.replace(reg, function (node, key) { return { 'userName': '${userName}','createDate':createDate}[key]; });
			...
			$("#submitBtn").click(function(){
                layer.confirm("确定要提交到下一步吗？", function(index) {
                    var mask = layer.msg('努力提交中', {icon: 16, time: 0, shade: [0.8, '#393D49']});
                    $.ajax({
                        url: "${ctx}/task/config/updateTaskStatus",
                        type: "POST",
                        data: {
                            "id" : "${task.id}"
                        },
			...
			$(".row-show").click(function(){
				...
				layer.open({
					...
					area: ['80%', '80%'],
					maxmin: true,
					content: '${ctx}/task/moduleActRow/rowDetail?id='+ row.attr("data-rowid") +'&type=${configType}' + '&taskId=${task.id}'
				});
			});
		...
		<script type="text/html" id="addCommentTemp">
			<tr class="comment-add">
     			...
     			<td><div class="comment-user">[userName]</div></td>
     			<td><div class="comment-date">[createDate]</div></td>
     			<td>
